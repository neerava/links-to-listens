...
- **Admin regenerate double-processing guard:** When the admin “Regenerate” action runs, `_failed_urls.add(source_url)` is called before the episode is deleted from the store. This prevents the watcher's poll loop from picking up the URL concurrently and processing it a second time. On success the URL is removed from `_failed_urls` (so `is_processed()` returns true and the watcher skips it); on failure `process_url` retains it in `_failed_urls` as normal.
